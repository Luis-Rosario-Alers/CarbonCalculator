...
class databasesModel(QObject):
    ...
    async def load_fuel_data(self, json_path):
        try:
    ...
    async def create_fuel_type_database(self, db_path):
        try:
            async with aiosqlite.connect(db_path) as conn:
                async with await conn.cursor() as cursor:
                    await cursor.execute(
    ...
    async def insert_fuel_data(self, db_path, fuel_data):
        try:
            async with aiosqlite.connect(db_path) as conn:
                async with await conn.cursor() as cursor:
                    for fuel in fuel_data:
    ...
    async def initialize_fuel_type_database(self):
        try:
            ...
            if os.path.exists(db_path):
                os.remove(db_path)
            json_path = await self.load_settings()
            if not json_path:
                return 0
            logger.debug(f"Loaded json path: {json_path}")

            fuel_data = await self.load_fuel_data(json_path)
            if not fuel_data:
                return 0
            logger.debug(f"Loaded fuel data: {fuel_data}")

            conn = await self.create_fuel_type_database(db_path)
            if not conn:
                return 0
            logger.debug("Database initialized")

            await self.insert_fuel_data(db_path, fuel_data)
    ...
    async def get_emissions_factor(self, fuel_type: str) -> int:
        db_path = os.path.join(databases_folder, "fuel_type_conversions.db")
        async with aiosqlite.connect(db_path) as conn:
            async with await conn.cursor() as cursor:
                await cursor.execute(
    ...
    async def log_calculation(self, user_id, fuel_type, fuel_used, emissions):
        try:
